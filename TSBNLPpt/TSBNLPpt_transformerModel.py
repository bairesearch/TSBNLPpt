...
# Copyright 2018 The Google AI Language Team Authors and The HuggingFace Inc. team.
# Copyright (c) 2018, NVIDIA CORPORATION.  All rights reserved.
# Copyright (c) 2022-2023 Baxter AI (baxterai.com)
#
...
if(transformerPOSembeddings):
	import TSBNLPpt_POSembedding
	def preparePOSdictionary():
		TSBNLPpt_POSembedding.preparePOSdictionary()
if(useGIAwordEmbeddings):
	import TSBNLPpt_GIAembedding
if(useTransformerRecursive):
	import TSBNLPpt_transformerRecursiveLayers
if(tokenMemoryBank):
	import TSBNLPpt_transformerTokenMemoryBank
if(transformerAttentionHeadPermutations):
	import TSBNLPpt_transformerAttentionHeadPermutations
...
class RobertaEmbeddings(nn.Module):
	...
	def __init__(self, config):
		super().__init__()
		if(transformerPOSembeddings):
			self.word_embeddings = TSBNLPpt_POSembedding.POSwordEmbeddingEncoderClass(config)
		elif(useGIAwordEmbeddings):
			self.word_embeddings = TSBNLPpt_GIAembedding.GIAwordEmbeddingEncoderClass(config)
		else:
			self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=config.pad_token_id)
		if(not relativeTimeEmbeddings):
		...
		if(not relativeTimeEmbeddings):
			...
			tokensMaxPositionEmbeddings = getMaxPositionEmbedding(sequenceRegisterContextualWindowLength)
			self.register_buffer("token_type_ids", torch.zeros([1, tokensMaxPositionEmbeddings], dtype=torch.long), persistent=False)
...
class RobertaSelfAttention(nn.Module):
	def __init__(self, config, robertaSharedLayerModules=None, position_embedding_type=None):
		...
		if relativeTimeEmbeddings:
			if self.position_embedding_type == "relative_time":
				...
				self.time_embedding = nn.Embedding(2 * config.max_position_embeddings - 1, self.attention_head_size)	  
		else:
		...
		if tokenMemoryBank:
			self.tokenMemoryBankClass = TSBNLPpt_transformerTokenMemoryBank.TokenMemoryBankClass(config)
	...
	def forward(
		...
		layerIndex=None,
		conceptColumnStartIndices=None,
		conceptColumnEndIndices=None,
		conceptColumnIDsPrev=None,
		conceptColumnIDsNext=None,
	) -> Tuple[torch.Tensor]:
		...
		attention_probs, context_layer = self.applySelfAttention(
			hidden_states, attention_mask, head_mask, query_layer, key_layer, value_layer
		)

		if localConceptColumnAttention:
			# Compute local concept column attention
			local_attention_probs, local_context_layer = TSBNLPpt_transformerConceptColumns.applyLocalConceptColumnAttention(self, hidden_states, attention_mask, head_mask, query_layer, key_layer, value_layer, conceptColumnStartIndices, conceptColumnEndIndices, conceptColumnIDsPrev, conceptColumnIDsNext)
			# Average the outputs
			context_layer = (context_layer + local_context_layer) / 2
			# Optionally, average attention_probs as well
			if output_attentions:
				attention_probs = (attention_probs + local_attention_probs) / 2
	...
	def calculateRelativePositionScores(self, hidden_states, query_layer, key_layer):
		if relativeTimeEmbeddings:
			if self.position_embedding_type == "relative_time":
				sequenceRegisterTokenTimes = self.tokenMemoryBankClass.getSequenceRegisterTokenTimes(
					self.tokenMemoryBankClass.sequenceRegisterMemoryBankTokenTimes,
					self.tokenMemoryBankClass.sequenceRegisterMemoryBankAccessTimes
				)
				...
				for sampleIndex in range(batchSize):  # do not parallel process samples as their token times are different
					...
					if transformerAttentionHeadPermutationsType == "dependent":
						positional_embedding = positional_embedding.repeat(
							self.num_attention_heads, self.num_attention_heads, 1
						)  # CHECKTHIS
...
class RobertaAttention(nn.Module):
	...
	def forward(
		...
		if tokenMemoryBank:
			hidden_states = self.self.tokenMemoryBankClass.loadSequenceRegisterHiddenStates(hidden_states)  # load additional hidden states from memory bank
		...
		self_outputs = self.self(
			...
			output_attentions,
			layerIndex,
			conceptColumnStartIndices=conceptColumnStartIndices,
			conceptColumnEndIndices=conceptColumnEndIndices,
			conceptColumnIDsPrev=conceptColumnIDsPrev,
			conceptColumnIDsNext=conceptColumnIDsNext,
		)

		attention_output = self.output(self_outputs[0], hidden_states)

		if tokenMemoryBank:
			attention_output = self.self.tokenMemoryBankClass.updateSequenceRegisterMemoryBank(layerIndex, attention_output)
...
class RobertaLayer(nn.Module):
	def __init__(self, config, robertaSharedLayerModules=None):
		...
		if localConceptColumnExperts:
			self.localConceptColumnExpertsTotal = config.localConceptColumnExpertsTotal
			# Create a list of "expert MLP" modules, one for each possible concept ID
			self.expertIntermediates = nn.ModuleList(
				[TSBNLPpt_transformerConceptColumns.ExpertIntermediate(config) for _ in range(self.localConceptColumnExpertsTotal)]
			)
			self.expertOutputs = nn.ModuleList(
				[TSBNLPpt_transformerConceptColumns.ExpertOutput(config) for _ in range(self.localConceptColumnExpertsTotal)]
			)
	...
	def forward(
		...
		if localConceptColumnExperts:
			if conceptColumnIDsPrev is not None:
				layer_output_exp1 = TSBNLPpt_transformerConceptColumns.feed_forward_experts(self, layer_output, conceptColumnIDsPrev)
				layer_output_exp2 = TSBNLPpt_transformerConceptColumns.feed_forward_experts(self, layer_output, conceptColumnIDsNext)
...
class RobertaEncoder(nn.Module):
	def __init__(self, config):
		...
		self.config = config
		if useTransformerRecursive:
			TSBNLPpt_transformerRecursiveLayers.declareRecursiveLayers(self, config, RobertaLayer)
		else:
					...
					for blockRecursiveIndex in range(recursiveLayersNumberIterations):
						...
						if self.gradient_checkpointing and self.training:
							...
							layer_outputs = torch.utils.checkpoint.checkpoint(
								...
								encoder_attention_mask,
								# i,
								conceptColumnStartIndices,
								conceptColumnEndIndices,
								conceptColumnIDsPrev,
								conceptColumnIDsNext,
							)
						else:
...
class RobertaModel(RobertaPreTrainedModel):
	...
	def forward(
		...
		if attention_mask is None:
			if tokenMemoryBank:
				# NOTUSED
				attention_mask = torch.ones(((batch_size, sequenceRegisterLength)), device=device)
			else:
		...
		if tokenMemoryBank:
			inputShape = [batchSize, sequenceRegisterLength]  # required?
			extended_attention_mask: torch.Tensor = self.get_extended_attention_mask(attention_mask, inputShape)
		else:
...
class RobertaForCausalLM(RobertaPreTrainedModel):
	...
	def forward(
		...
		past_key_values (`tuple(tuple(torch.FloatTensor))` of length `config.n_layers` with each tuple having 2 tensors of shape `(batch_size, num_heads, sequence_length - 1, embed_size_per_head)`):
...
#TSBNLPpt_transformerModel specific code;
